// === Lesson/Practice Unit Recall Logic ===
document.addEventListener("DOMContentLoaded", () => {
  // (Moved lesson/unit recall logic to after async data loads)

  // --- Note Colors UI Logic ---
  fetch("data/noteColors.json")
    .then((r) => r.json())
    .then((colors) => {
      const container = document.getElementById("note-colors-list");
      if (!container) return;
      // Load user meanings from localStorage
      let colorNotes = {};
      try {
        colorNotes =
          JSON.parse(localStorage.getItem("noteColorMeanings") || "{}") || {};
      } catch {}
      // Render color swatches and text boxes
      container.innerHTML = "";
      colors.forEach((color) => {
        ...
        swatch.className =
          "inline-block w-6 h-6 rounded border border-gray-400";
        swatch.style.background = color;
        swatch.title = color;
        // Text box
        const input = document.createElement("input");
        input.type = "text";
        input.className = "border rounded px-2 py-1 text-sm w-48 pr-6";
        input.placeholder = `What does ${color} mean?`;
        input.value = colorNotes[color] || "";
        input.addEventListener("change", () => {
          colorNotes[color] = input.value;
          localStorage.setItem("noteColorMeanings", JSON.stringify(colorNotes));
        });
        // X icon for clearing
        const clearBtn = document.createElement("button");
        clearBtn.type = "button";
        clearBtn.innerHTML = "&#10006;"; // Unicode X
        clearBtn.title = "Clear this meaning";
        clearBtn.className =
          "absolute right-2 text-gray-400 hover:text-red-500 focus:outline-none text-xs";
  ...
  // Reset Colors button logic (refactored)
  function resetNoteColors() {
    fetch("data/noteColors.json")
      .then((r) => r.json())
      .then((colors) => {
        if (!window._noteColorState) window._noteColorState = {};
        // Remove all keys not in colors
        Object.keys(window._noteColorState).forEach((k) => {
          if (!colors.includes(k)) delete window._noteColorState[k];
        });
        // Set all color keys to black
        colors.forEach((c) => (window._noteColorState[c] = "black"));
        // Do NOT clear user meanings or text boxes
        // Re-render the scale so black colors are shown
        if (typeof renderCurrentScale === "function") {
          renderCurrentScale();
        }
      });
  }
  const resetBtn = document.getElementById("reset-note-colors-btn");
  if (resetBtn) {
    resetBtn.onclick = resetNoteColors;
  }
});
// === Snapshot Save/Recall Logic ===
document.addEventListener("DOMContentLoaded", () => {
  const saveBtn = document.getElementById("save-snapshot-btn");
  const loadBtn = document.getElementById("load-snapshot-btn");
  const loadInput = document.getElementById("load-snapshot-input");
  ...
  if (saveBtn) {
    saveBtn.onclick = function () {
      ...
      let name = "Scale_";
      if (snapshot.practiceType === "passages" && snapshot.passageName) {
        name = `Passage_${snapshot.passageName.replace(/\s+/g, "-")}`;
      } else if (snapshot.scaleName) {
        name = `Scale_${snapshot.scaleName.replace(/\s+/g, "-")}`;
      }
      // Save to localStorage as well as download
      let units = JSON.parse(localStorage.getItem("userPracticeUnits") || "[]");
      units.push({ name, snapshot });
      localStorage.setItem("userPracticeUnits", JSON.stringify(units));

      // Only save to localStorage, no file download
      statusDiv.textContent = `Snapshot saved in your browser storage as a Practice Unit.`;
    };
  }

  if (loadBtn) {
    loadBtn.onclick = function () {
      // Show a list of saved Practice Units in localStorage
      let units = JSON.parse(localStorage.getItem("userPracticeUnits") || "[]");
      if (!units.length) {
        statusDiv.textContent = "No Practice Units found in browser storage.";
        return;
      }
      // Simple prompt for now; can be replaced with a modal/UI
      const names = units.map((u, i) => `${i + 1}: ${u.name}`).join("\n");
      const idx = prompt(
        `Select a Practice Unit to load (enter number):\n${names}`
      );
      const n = parseInt(idx, 10);
      if (!isNaN(n) && n > 0 && n <= units.length) {
        applyWorkspaceSnapshot(units[n - 1].snapshot);
        statusDiv.textContent = `Snapshot loaded: ${units[n - 1].name}`;
      } else {
        statusDiv.textContent = "Load cancelled or invalid selection.";
      }
    };
  }
});
...
function collectWorkspaceSnapshot() {
  ...
  const accidentalFamily = document.querySelector(
    'input[name="accidental-family"]:checked'
  )?.value;
  const showKeySignature =
    document.getElementById("show-key-signature")?.checked;
  const showAccidentals = document.getElementById("show-accidentals")?.checked;
  const showBarLines = document.getElementById("show-bar-lines")?.checked;
  const showTimeSignature = document.getElementById(
    "show-time-signature"
  )?.checked;
  const showOverlaysTooltip = document.getElementById(
    "show-overlays-tooltip"
  )?.checked;
  const startingOctave = document.getElementById(
    "starting-octave-label"
  )?.textContent;
  const displayOption = document.querySelector(
    'input[name="display-option"]:checked'
  )?.value;
  const practiceType = document.querySelector(
    'input[name="practice-type"]:checked'
  )?.value;
  const passageName =
    document.getElementById("passage-select")?.selectedOptions?.[0]
      ?.textContent;
  const passageFile = document.getElementById("passage-select")?.value;

  // Compose a snapshot object
  // Also save per-note color state if present
  let noteColorState = window._noteColorState || {};
  return {
    instrument,
    rootNote,
    scaleType,
    ...
    passageName,
    passageFile,
    scaleName: scaleType ? `${rootNote} ${scaleType}` : undefined,
...
function applyWorkspaceSnapshot(snapshot) {
  // Restore per-note color state
  if (snapshot.noteColorState) {
    window._noteColorState = { ...snapshot.noteColorState };
  } else {
    window._noteColorState = {};
  }
  // Restore all relevant UI state
  ...
  if (snapshot.accidentalFamily) {
    const af = document.querySelector(
      `input[name='accidental-family'][value='${snapshot.accidentalFamily}']`
    );
    if (af) af.checked = true;
  }
  if (typeof snapshot.showKeySignature === "boolean")
    document.getElementById("show-key-signature").checked =
      snapshot.showKeySignature;
  if (typeof snapshot.showAccidentals === "boolean")
    document.getElementById("show-accidentals").checked =
      snapshot.showAccidentals;
  if (typeof snapshot.showBarLines === "boolean")
    document.getElementById("show-bar-lines").checked = snapshot.showBarLines;
  if (typeof snapshot.showTimeSignature === "boolean")
    document.getElementById("show-time-signature").checked =
      snapshot.showTimeSignature;
  if (typeof snapshot.showOverlaysTooltip === "boolean")
    document.getElementById("show-overlays-tooltip").checked =
      snapshot.showOverlaysTooltip;
  if (snapshot.startingOctave) {
    // Try to set starting octave if possible
    const label = document.getElementById("starting-octave-label");
    if (label) label.textContent = snapshot.startingOctave;
  }
  if (snapshot.displayOption) {
    const disp = document.querySelector(
      `input[name='display-option'][value='${snapshot.displayOption}']`
    );
    if (disp) disp.checked = true;
  }
  if (snapshot.practiceType) {
    const pt = document.querySelector(
      `input[name='practice-type'][value='${snapshot.practiceType}']`
    );
    if (pt) pt.checked = true;
  }
  if (snapshot.passageFile) {
    const passageDropdown = document.getElementById("passage-select");
    passageDropdown.value = snapshot.passageFile;
    // Fire change event to ensure passage data loads and UI updates
    const event = new Event("change", { bubbles: true });
    passageDropdown.dispatchEvent(event);
  }
  // Trigger re-render
  renderCurrentScale && renderCurrentScale();
}
// show-scales.js
...
fetch("data/pitch-class.json")
  ...
  .then((text) => {
    ...
    // ---- DEFERRED LESSON/UNIT RECALL LOGIC ----
    const urlParams = new URLSearchParams(window.location.search);
    const lessonName = urlParams.get("lesson");
    const unitParam = urlParams.get("unit");
    const lessonRaw = sessionStorage.getItem("activeLesson");
    if (lessonRaw) {
      try {
        const lesson = JSON.parse(lessonRaw);
        let unitToLoad = null;
        if (unitParam && lesson.units && lesson.units.length > 0) {
          // Try to match by name (case-sensitive, exact)
          unitToLoad = lesson.units.find((u) => u.name === unitParam);
          if (!unitToLoad) {
            // Try by index if unitParam is a number
            const idx = parseInt(unitParam, 10);
            if (!isNaN(idx) && idx >= 0 && idx < lesson.units.length) {
              unitToLoad = lesson.units[idx];
            }
          }
        }
        if (!unitToLoad && lesson.units && lesson.units.length > 0) {
          unitToLoad = lesson.units[0]; // fallback to first
        }
        if (unitToLoad) {
          // Support both {snapshot: object} and {content: string} shapes
          let snapshot = null;
          if (unitToLoad.snapshot) {
            snapshot = unitToLoad.snapshot;
          } else if (unitToLoad.content) {
            try {
              snapshot = JSON.parse(unitToLoad.content);
            } catch (err) {
              console.error("Failed to parse Practice Unit content:", err);
            }
          }
          if (snapshot) {
            ...
            if (title) {
              title.textContent = `🎼 Lesson: ${lesson.name} — ${
                unitToLoad.name || "Practice Unit"
              }`;
            }
          } else {
            console.warn(
              "[Lesson Recall] No snapshot found for unit:",
              unitToLoad.name
            );
          }
        } else {
          console.warn(
            "[Lesson Recall] No unitToLoad found for param:",
            unitParam
          );
        }
      } catch (err) {
        console.error("Failed to parse lesson from sessionStorage:", err);
      }
    }
    // ---- END DEFERRED LESSON/UNIT RECALL LOGIC ----
  })
  .catch((error) => {
...
function attachEventListeners() {
  ...
  // Overlay tooltip checkbox event
  const overlayTooltipCheckbox = document.getElementById(
    "show-overlays-tooltip"
  );
  if (overlayTooltipCheckbox) {
    overlayTooltipCheckbox.addEventListener("change", function () {
      renderCurrentScale();
    });
  }
}
...
function renderCurrentScale() {
  ...
  if (typeof renderScale === "function") {
    ...
    // Pass per-note color state and update callback
    renderScale(
      entry,
      scaleType,
      rootNote,
      ...
      showBarLines,
      window._noteColorState || {},
      (noteIdx, nextColorIdx) => {
        // Update color state and re-render
        if (!window._noteColorState) window._noteColorState = {};
        window._noteColorState[noteIdx] = nextColorIdx;
        renderCurrentScale();
      }
    );
  }
  ...
  // After rendering overlays, attach tooltips if needed
  console.log("[TOOLTIP CALL] setTimeout(attachOverlayTooltips, 0)");
  setTimeout(attachOverlayTooltips, 0);
  usableOctaveRange = getUsableOctaveRange(entry, rootNote, octaveCount);
}
...
function renderPassageStave(
  ...
  diagnosticLog(
    `🎵 Stave ${staveIndex + 1} Notes:`,
    allNotesForStave.map((n) => (n.keys ? n.keys.join(",") : n.duration))
  );
  ...
  window._lastRenderedNotes = allNotesForStave; // <-- Add this line DRG
  // Always schedule tooltip attachment after overlays for passages
  console.log("[TOOLTIP CALL] setTimeout(attachOverlayTooltips, 0) [passage]");
  setTimeout(attachOverlayTooltips, 0);
...
function addPassageDisplayOverlays(
  notes,
  displayMode,
  entry,
  tooltipOnly = false
  ...
  notes.forEach((vfNote) => {
    if (vfNote._isRest || vfNote.duration?.endsWith("r")) return;
...
// --- Overlay Tooltip Mouse Logic ---
function attachOverlayTooltips() {
  console.log("[TOOLTIP ENTRY] attachOverlayTooltips() called");
  // Remove any previous tooltips
  const old = document.getElementById("note-overlay-tooltip");
  if (old) old.remove();

  // Only attach if tooltip-only is enabled
  const tooltipOnly = document.getElementById("show-overlays-tooltip")?.checked;
  if (!tooltipOnly) return;

  // Find all SVG noteheads and map to overlays
  const svg = document.querySelector("#vf svg");
  if (!svg) return;
  // Support both g.vf-note and g.vf-stavenote for different renderers
  let noteGroups = Array.from(
    svg.querySelectorAll("g.vf-note, g.vf-stavenote")
  );
  console.log(
    "[Tooltip Debug] Found noteGroups:",
    noteGroups.length,
    noteGroups
  );
  let overlays = [];
  if (window._lastRenderedNotes && Array.isArray(window._lastRenderedNotes)) {
    overlays = window._lastRenderedNotes.map((n) => n._overlayTooltip || null);
  }
  console.log("[Tooltip Debug] overlays:", overlays);
  noteGroups.forEach((g, i) => {
    const overlay = overlays[i];
    console.log(
      `[Tooltip Debug > Problem here >>] Mapping group #${i} to overlay:`,
      overlay
    );
    if (!overlay) return;
    ...
    g.addEventListener("mouseenter", (e) => {
      ...
      tooltip.id = "note-overlay-tooltip";
      tooltip.textContent = overlay;
      ...
      g.addEventListener("mouseleave", function cleanup() {
        tooltip.remove();
        g.removeEventListener("mousemove", moveTooltip);
        g.removeEventListener("mouseleave", cleanup);
      });
    });
  });
}
