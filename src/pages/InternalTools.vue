  ...
  import { listTopologies, saveTopology, fetchTopologyById, deleteTopology } from '../_network_interconnect/supabaseStore.js'
  import { isSupabaseConfigured } from '../_network_interconnect/supabaseClient.js'
  ...
  function buildTextTreeWithDebug(devices, connections) {
    ...
    if (!roots.length) {
      ...
      const asciiTreeString = treeString;
      const debugTree = devices.map(d => ({ name: idToName[d.id || d.name], label: '', children: [] }));
      return { treeString, asciiTreeString, debugTree };
    }
    ...
    const unicodeTree = lines.join('\n');
    const asciiTreeString = unicodeTree
      .replaceAll('└── ', '`-- ')
      .replaceAll('├── ', '|-- ')
      .replaceAll('│   ', '|   ');
    return { treeString: unicodeTree, asciiTreeString, debugTree };
  }
  ...
  const textDiagram = ref({ treeString: '', asciiTreeString: '', debugTree: null });
  ...
  async function reloadData() {
    ...
    mermaidSvg.value = '';
    mermaidError.value = '';
    textDiagram.value = { treeString: '', asciiTreeString: '', debugTree: null };
  ...
  async function handleGenerate() {
    ...
    const { treeString, asciiTreeString, debugTree } = buildTextTreeWithDebug(devices, connections);
    textDiagram.value = { treeString, asciiTreeString, debugTree };
  ...
  async function refreshSaved() {
    if (!supabaseConfigured.value) {
      supabaseError.value = 'Supabase not configured';
    ...
    supabaseError.value = '';
    supabaseStatus.value = 'Loading saved items…';
    try {
      const { data, error } = await listTopologies();
      if (error) throw error;
      savedList.value = data || [];
      supabaseStatus.value = `Loaded ${savedList.value.length} item(s).`;
    } catch (e) {
      supabaseError.value = String(e.message || e);
      supabaseStatus.value = '';
    } finally {
  ...
  async function onSaveToSupabase() {
    ...
    try { parsed = JSON.parse(jsonText.value); }
      ...
      supabaseError.value = 'Invalid JSON: ' + (e.message || e);
      supabaseStatus.value = '';
      return;
    }
    saving.value = true;
    try {
      ...
      const { data, error } = await saveTopology(name, parsed);
      if (error) throw error;
      supabaseStatus.value = `Saved as "${data?.name}".`;
      supabaseError.value = '';
      await refreshSaved();
    } catch (e) {
      supabaseError.value = String(e.message || e);
  ...
  async function onLoadFromSupabase() {
    if (!supabaseConfigured.value) {
      supabaseError.value = 'Supabase not configured';
      supabaseStatus.value = '';
      return;
    ...
    try {
      const { data, error } = await fetchTopologyById(selectedTopologyId.value);
      if (error) throw error;
      if (data?.data) {
        jsonText.value = JSON.stringify(data.data, null, 2);
        loadTablesFromJson();
        supabaseStatus.value = `Loaded "${data.name}".`;
        supabaseError.value = '';
      }
    } catch (e) {
      supabaseError.value = String(e.message || e);
  ...
  async function onDeleteFromSupabase() {
    if (!supabaseConfigured.value) {
      supabaseError.value = 'Supabase not configured';
      supabaseStatus.value = '';
      return;
    ...
    if (!selectedTopologyId.value) return;
    try {
      const { error } = await deleteTopology(selectedTopologyId.value);
      if (error) throw error;
      supabaseStatus.value = 'Deleted.';
      supabaseError.value = '';
      ...
      <main class="container mx-auto p-4 flex-1">
          ...
          <div>
            <div class="mb-2 flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button class="btn btn-xs btn-outline" @click="copyJson" :disabled="!jsonText">Copy JSON</button>
            ...
            <div class="mt-2 flex gap-2">
              <button class="btn btn-sm btn-primary" @click="handleValidate" :disabled="loading">Validate</button>
              <button class="btn btn-sm btn-secondary" @click="handleGenerate" :disabled="loading || (errors && errors.length && errors[0] !== 'JSON is valid!')">Generate</button>
            </div>

            <details class="mt-3 border border-base-300 rounded">
              <summary class="px-3 py-2 cursor-pointer font-semibold flex items-center justify-between">
                <span>Save / Recall from Supabase</span>
                <div class="flex gap-2">
                  <button class="btn btn-xs" @click.stop="refreshSaved" :disabled="!supabaseConfigured">Refresh</button>
                </div>
              </summary>
              <div class="p-3 space-y-3 bg-base-100 text-xs">
                <div v-if="!supabaseConfigured" class="text-error">Supabase not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY (or window.__SUPABASE_URL__/__SUPABASE_ANON_KEY__).</div>
                <div v-else-if="supabaseStatus" class="text-info">{{ supabaseStatus }}</div>
                <div v-if="supabaseError && supabaseConfigured" class="text-error">{{ supabaseError }}</div>
                <div class="flex gap-2 items-end">
                  <label class="form-control w-full">
                    <span class="label-text text-xs">Name</span>
                    <input type="text" v-model="saveName" class="input input-bordered input-sm w-full" placeholder="e.g., Home Lab - Closet" />
                  </label>
                  <button class="btn btn-sm btn-success" @click="onSaveToSupabase" :disabled="!jsonText || saving || !supabaseConfigured">{{ saving ? 'Saving…' : 'Save' }}</button>
                </div>
                <div class="flex gap-2 items-end">
                  <label class="form-control w-full">
                    <span class="label-text text-xs">Saved Topologies</span>
                    <select class="select select-bordered select-sm w-full" v-model="selectedTopologyId">
                      <option disabled value="">Select a saved item…</option>
                      <option v-for="item in savedList" :key="item.id" :value="item.id">{{ item.name }} — {{ item.updated_at?.slice(0,10) }}</option>
                    </select>
                  </label>
                  <button class="btn btn-sm btn-primary" @click="onLoadFromSupabase" :disabled="!selectedTopologyId || !supabaseConfigured">Load</button>
                  <button class="btn btn-sm btn-outline btn-error" @click="onDeleteFromSupabase" :disabled="!selectedTopologyId || !supabaseConfigured">Delete</button>
                </div>
              </div>
            </details>
        ...
        <details class="mt-4 border border-base-300 rounded w-full" :open="showTableEditor">
          <summary class="px-3 py-2 cursor-pointer font-semibold flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button class="btn btn-xs btn-outline" @click.stop="copyJson" :disabled="!jsonText">Copy JSON</button>
              <span class="text-xs text-gray-500">collapsible</span>
            </div>
          </summary>
          <div class="p-3 space-y-3 bg-base-100">
            <div class="flex gap-2 flex-wrap items-center">
              <button class="btn btn-sm btn-outline" @click="loadTablesFromJson">Load from JSON</button>
              <button class="btn btn-sm btn-primary" @click="syncJsonFromTables">Sync to JSON</button>
              <label class="btn btn-sm btn-outline" for="json-upload">Upload JSON</label>
              ...
              <div class="flex items-center gap-1">
                ...
                />
                <button class="btn btn-sm btn-accent" @click="saveJsonToFile">Save As</button>
              </div>
              <button class="btn btn-sm btn-outline" @click="addDeviceRow">Add device</button>
              <button class="btn btn-sm btn-outline" @click="addConnectionRow">Add connection</button>
              <input
                v-model="tableFilter"
                type="text"
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <input
                  ...
                  class="input input-xs input-bordered w-32"
                />
                <button class="btn btn-sm btn-outline" @click="applyGlobalReplace">Find & Replace</button>
                ...
                <table class="table table-xs w-full">
                  ...
                  <tbody>
                    <tr v-for="(d, idx) in filteredDevices" :key="idx">
                      ...
                      <td>
                        <button class="btn btn-xs btn-ghost" @click="duplicateDeviceRow(idx)">Dup</button>
                        <button class="btn btn-xs btn-ghost text-error" @click="deleteDeviceRow(idx)">Del</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
                ...
                <table class="table table-xs w-full">
                  ...
                  <tbody>
                    <tr v-for="(c, idx) in filteredConnections" :key="idx">
                      ...
                      <td>
                        <button class="btn btn-xs btn-ghost" @click="duplicateConnectionRow(idx)">Dup</button>
                        <button class="btn btn-xs btn-ghost text-error" @click="deleteConnectionRow(idx)">Del</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
        ...
        <div class="mt-8 space-y-4">
          ...
          <details class="border border-base-300 rounded">
            ...
            <div class="p-3">
              <div class="overflow-auto">
                <table class="table table-xs w-full border border-base-300" style="border-collapse: collapse;">
                  <thead>
                    <tr>
                      <th class="border border-base-300">From (id)</th>
                      <th class="border border-base-300">From (name)</th>
                      <th class="border border-base-300">To (id)</th>
                      <th class="border border-base-300">To (name)</th>
                      <th class="border border-base-300">Type</th>
                      <th class="border border-base-300">From Label</th>
                      <th class="border border-base-300">To Label</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-if="!interconnectRows.length">
                      <td colspan="7" class="text-center text-gray-400 border border-base-300">Generate to populate this table.</td>
                    </tr>
                    <tr v-for="(row, idx) in interconnectRows" :key="idx">
                      <td class="border border-base-300">{{ row.fromId }}</td>
                      <td class="border border-base-300">{{ row.fromName }}</td>
                      <td class="border border-base-300">{{ row.toId }}</td>
                      <td class="border border-base-300">{{ row.toName }}</td>
                      <td class="border border-base-300">{{ row.type }}</td>
                      <td class="border border-base-300">{{ row.fromLabel }}</td>
                      <td class="border border-base-300">{{ row.toLabel }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          <details class="border border-base-300 rounded">
            ...
            <div class="p-3 space-y-3">
              ...
              <details class="border border-dashed border-base-300 rounded">
                <summary class="px-2 py-1 cursor-pointer text-xs">Text Tree Diagnostics</summary>
                <div class="p-2 space-y-2">
                  <div>
                    <div class="text-xs font-semibold mb-1">ASCII Fallback Preview</div>
                    <div class="bg-base-100 border border-base-300 rounded p-2 overflow-auto font-mono text-xs">
                      <pre style="white-space: pre">{{ textDiagram.asciiTreeString }}</pre>
                    </div>
                  </div>
                  <div>
                    <div class="text-xs font-semibold mb-1">Debug Tree JSON</div>
                    <div class="bg-base-100 border border-base-300 rounded p-2 overflow-auto font-mono text-xs">
                      <pre style="white-space: pre">{{ JSON.stringify(textDiagram.debugTree, null, 2) }}</pre>
                    </div>
                  </div>
                </div>
              </details>
